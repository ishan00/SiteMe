...
styles=['image','link','list','piechart','table','slideshow','parallax','fade','card','button', 'accordion', 'timeline' , 'chatbox','checkbox']
...
t_GRID=r'grid\([\s\S]*?\)(\{[^\{\}]*[\n ]*([^\{\}]*[\n ]*\{[^\{\}]*[\n ]*\}[^\{\}]*[\n ]*)*[^\{\}]*[\n ]*\})+'
t_HRULE=r'-{5,}'
t_REST=r'(?:(?!([#]*('+'|'.join(keywords)+'):[0-9a-zA-Z ][a-zA-Z0-9() ,]*|('+'|'.join(styles)+')?\([^\(\)\{\}]*?\)\{[^\(\)\{\}]*?\}|grid\([\s\S]*?\)(\{[\s\S]*?\})*|\(Pre\)\{\{[\s\S]*?\}\}|-{5,})).)+'
t_NEWLINE=r'\n\r|\r\n|\r|\n'
